...
def darknet_v1(input_tensor, activation, **kwargs):
    ...
    x = conv_layer(x, 1024, (3, 3), strides=(1, 1), activation=activation, name='block4_6')
...
def darknet_v3(input_tensor, activation, **kwargs):
    ...
    x = conv_layer(x, 64, (3, 3), strides=(2, 2), padding='valid', activation=activation, **kwargs)

    x = darknet_res_block(x, 32, activation=activation, name='block1_1', **kwargs)
    x = conv_layer(x, 128, (3, 3), strides=(2, 2), padding='valid', activation=activation, **kwargs)

    for i in range(2):
        x = darknet_res_block(x, 64, (3, 3), activation=activation, name='block2_' + str(i), **kwargs)
    x = conv_layer(x, 256, (3, 3), strides=(2, 2), padding='valid', activation=activation, **kwargs)

    for i in range(8):
        x = darknet_res_block(x, 128, (3, 3), activation=activation, name='block3_' + str(i), **kwargs)
    scale1 = x
    x = conv_layer(x, 256, (3, 3), strides=(2, 2), padding='valid', activation=activation, **kwargs)

    for i in range(8):
        x = darknet_res_block(x, 256, (3, 3), activation=activation, name='block4_' + str(i), **kwargs)
    scale2 = x
    x = conv_layer(x, 1024, (3, 3), strides=(2, 2), padding='valid', activation=activation, **kwargs)

    for i in range(4):
        x = darknet_res_block(x, 512, (3, 3), activation=activation, name='block4_' + str(i), **kwargs)
