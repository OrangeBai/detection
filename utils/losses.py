...
def yolo_v1_loss(box_num, weights, lambda_co=5, lambda_no=0.5):
    def yolo_v1_loss_fun(y_gt, y_pre):
        ...
        for box_id in range(box_num):
            ...
            loss_coord += bk.sum(pos * bk.sum(bk.square(bk.sqrt(y_gt[:, :, :, box_st + 3: box_st + 5]) -
                                                        bk.sqrt(y_pre[:, :, :, box_st + 3: box_st + 5])), axis=-1))
        ...
        return loss_coord * lambda_co + loss_none * lambda_no + loss_obj + loss_class
